...
\end{proof}
\begin{crly}
    Let $X$ be an irreducible projective variety. Then all reglar functions on $X$ are constant.
\end{crly}
\begin{proof}
...
\begin{defn}
    A rational map $f:X\to Y$ of irreducible projective varieties is dominant if $f(X)$ is dense in $Y$.
\end{defn}
A rational map is birational if it has rational inverse. In this case  there exists $U$ open dense in $X$ nad $V$ open dense in $Y$ s.t. $U$ is isomorphic to  $V$.
