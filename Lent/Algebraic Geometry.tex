...
\end{lem}
\begin{proof}
    Note taht if $\omega,\omega'\in\Omega_V^\times$, then write $\omega=hd\pi_P$, $\omega'=h'd\pi_P$, have $\nu_P(\omega+\omega')=\nu_P(h+h')\ge\min\{\nu_P(h),\nu_P(h')\}=\min_{\nu_P(\omega),\nu_P(\omega')}$.
